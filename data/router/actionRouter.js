...
router.get("/", (req, res) => {
  Action.get()
    ...
    .catch(err => {
      res.status(404).json({ errMessage: "Error getting actions" });
    });
});
...
router.post("/", validateActionPost, (req, res) => {
  ...
  Action.insert(post)
    ...
    .catch(err => {
      ...
      res.status(404).json({ errMessage: "Error adding action" });
    });
});
...
router.put("/:id", validateActionId, (req, res) => {
  ...
  Action.update(id, changes)
    ...
    .catch(err => {
      console.log(err);
      res.status(404).json({ error: "Error getting actions" });
    });
});
...
function validateActionPost(req, res, next) {
  ...
  if (!description) {
    return res.status(404).json({ errMessage: "a description has no text" });
  }
  if (!notes) {
    return res.status(404).json({ errMessage: "a notes has no text" });
  }
  next();
}
