...
router.get("/", (req, res) => {
  Project.get()
    ...
    .catch(err => {
      res.status(404).json({ errMessage: "Error getting projects" });
    });
});
...
router.post("/", validateProjectPost, (req, res) => {
  ...
  Project.insert(projects)
    ...
    .catch(err => {
      ...
      res.status(404).json({ errMessage: "Error adding project" });
    });
});
...
function validateProjectPost(req, res, next) {
  ...
  if (!name) {
    return res.status(404).json({ errMessage: "a name has no text" });
  }
  next();
}
